...
class SignIn extends Component {
		...
		if (!emailIsValid) {
			...
			if (title === 'Login') {
				if (!passwordIsValid) {
					...
					//login
					setLoading(true)
					apiRequest('POST', '/sign-in', {email, password})
						.then(resData => {
							setLoading(false)
    ...
    onVerifyEmail = () => {
    	...
    	if (passwordIsValid && password === password2) {
    		setLoading(true)
			apiRequest('POST', '/register/complete', {password, token} )
			.then(resData => {
				setLoading(false)
				if (resData && resData.error != null) {
					const { name, title, message } = resData.error;
    ...
    handleVerification = (email) => {
    	...
    	if (!/\S/.test(name)) {
	    	...
	    	setLoading(true)
				apiRequest('POST', '/register', {name, email})
					.then(userData => {
						setLoading(false)
    ...
    responseFacebook = (response) => {
    	...
    	if (response && response.email) {
    		...
    		setLoading(true)
			apiRequest('POST', '/api/v1/auth/facebook', {accessToken, id, email, name})
				.then(userData => {
					if (userData && userData.error != null) {
						const { title, message } = userData.error;
						optionAlert({
